...
// KafkaClient owns the manual-commit consumer and all-ISR-acknowledged DLQ producer.
...
func NewKafkaClient(ctx context.Context, kafkaConfig kafkaclient.Config, config KafkaClientConfig) (*KafkaClient, error) {
	...
	if config.GroupID == "" || len(config.GroupID) > 128 || strings.ContainsAny(config.GroupID, "\r\n\x00") {
		return nil, errors.New("order enrichment Kafka group ID is invalid")
	}
	if config.SessionTimeout <= 0 || config.HeartbeatInterval <= 0 || config.HeartbeatInterval >= config.SessionTimeout {
		return nil, errors.New("order enrichment Kafka session and heartbeat intervals are invalid")
	}
	...
	options = append(options,
		...
		kgo.ConsumeTopics(eventcontract.OrderEnrichmentTopicV1),
	...
	if err != nil {
		return nil, fmt.Errorf("create order enrichment Kafka client: %w", err)
	}
	if err := client.Ping(ctx); err != nil {
		client.Close()
		return nil, fmt.Errorf("ping Kafka for order enrichment consumer: %w", err)
	}
...
func (client *KafkaClient) AllowRebalance() {
...
func (client *KafkaClient) CommitRecord(ctx context.Context, record *kgo.Record) error {
	if client == nil || client.client == nil || record == nil {
		return errors.New("order enrichment Kafka commit client and record are required")
	}
	if err := client.client.CommitRecords(ctx, record); err != nil {
		return fmt.Errorf("commit order enrichment Kafka record %s: %w", recordPosition(record), err)
	}
	return nil
}

type consumerDeadLetterEnvelope struct {
...
func (client *KafkaClient) ProduceDeadLetter(ctx context.Context, source *kgo.Record, errorClass string, cause error, failedAt time.Time) error {
	if client == nil || client.client == nil {
		return errors.New("order enrichment DLQ client is required")
	}
	...
	if _, err := client.client.ProduceSync(ctx, record).First(); err != nil {
		return fmt.Errorf("produce order enrichment dead letter for %s: %w", recordPosition(source), err)
	}
	return nil
}

// BuildDeadLetterRecord preserves source identity and bounded payload evidence without exposing an unbounded failure.
func BuildDeadLetterRecord(source *kgo.Record, errorClass string, cause error, failedAt time.Time) (*kgo.Record, error) {
	if source == nil || cause == nil || failedAt.UnixMilli() <= 0 || source.Topic == "" || source.Partition < 0 || source.Offset < 0 {
		return nil, errors.New("order enrichment DLQ source, cause, and time are required")
	}
	messageID := sourceHeader(source, eventcontract.DomainHeaderMessageID)
	if !validText(messageID, 128) {
		messageID = recordPosition(source)
	}
	...
	envelope := consumerDeadLetterEnvelope{
		SchemaVersion:        1,
		...
		SourcePayloadSample:  append([]byte(nil), payloadSample...),
		SourcePayloadBytes:   len(source.Value),
		SourcePayloadSHA256:  hex.EncodeToString(digest[:]),
		SourcePayloadTrimmed: trimmed,
		ErrorClass:           safeErrorClass(errorClass),
		Failure:              sanitizeFailure(cause.Error()),
		FailedAtUnixMs:       failedAt.UnixMilli(),
	}
	payload, err := json.Marshal(envelope)
	if err != nil {
		return nil, fmt.Errorf("marshal order enrichment dead letter: %w", err)
	}
	return &kgo.Record{
