...
func NewKafkaClient(ctx context.Context, kafkaConfig kafkaclient.Config, config KafkaClientConfig) (*KafkaClient, error) {
	...
	if !validText(config.GroupID, 128) || config.SessionTimeout <= 0 || config.HeartbeatInterval <= 0 || config.HeartbeatInterval >= config.SessionTimeout {
		return nil, errors.New("pgvector Kafka group and heartbeat configuration is invalid")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("create pgvector Kafka client: %w", err)
	}
	if err := client.Ping(ctx); err != nil {
		client.Close()
		return nil, fmt.Errorf("ping Kafka for pgvector consumer: %w", err)
	}
...
func (client *KafkaClient) CommitRecord(ctx context.Context, record *kgo.Record) error {
	if client == nil || client.client == nil || record == nil {
		return errors.New("pgvector Kafka commit client and record are required")
	}
	if err := client.client.CommitRecords(ctx, record); err != nil {
		return fmt.Errorf("commit pgvector Kafka record %s: %w", recordPosition(record), err)
	}
	return nil
}
...
func (client *KafkaClient) ProduceDeadLetter(ctx context.Context, source *kgo.Record, errorClass string, cause error, failedAt time.Time) error {
	if client == nil || client.client == nil {
		return errors.New("pgvector DLQ client is required")
	}
	...
	if _, err := client.client.ProduceSync(ctx, record).First(); err != nil {
		return fmt.Errorf("produce pgvector dead letter for %s: %w", recordPosition(source), err)
	}
	return nil
}
...
func BuildDeadLetterRecord(source *kgo.Record, errorClass string, cause error, failedAt time.Time) (*kgo.Record, error) {
	if source == nil || cause == nil || failedAt.UnixMilli() <= 0 || source.Topic == "" || source.Partition < 0 || source.Offset < 0 {
		return nil, errors.New("pgvector DLQ source, cause, and time are required")
	}
	...
	envelope := deadLetterEnvelope{
		SchemaVersion: 1, Consumer: "index-pgvector", SourceTopic: source.Topic,
	...
	if err != nil {
		return nil, fmt.Errorf("marshal pgvector dead letter: %w", err)
	}
	return &kgo.Record{
...
func sanitizeFailure(value string) string {
	...
	if value == "" {
		return "unknown vector index failure"
	}
	return value
}
