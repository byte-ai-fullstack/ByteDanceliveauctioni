...
-- 11 room display lot.
-- ARGV:
-- 1 event_id, 2 trace_id, 3 reason, 4 operator_id, 5 cancelled_status,
...
local reason = string.match(ARGV[3], '^%s*(.-)%s*$')
local operator_id = string.match(ARGV[4], '^%s*(.-)%s*$')
local cancelled_status = tonumber(ARGV[5])
...
if not valid_uuid_v7(event_id) or room_id == '' or reason == '' or operator_id == '' then
  return reject('INVALID_ARGUMENT')
end
if previous_status == 3 or previous_status == 4 or previous_status == 8 then
  return reject('ALREADY_TERMINAL')
end
if previous_status ~= 1 and previous_status ~= 2 and previous_status ~= 5 and previous_status ~= 6 and previous_status ~= 7 then
  return reject('RUNTIME_STATE_CORRUPT')
end
...
if not exact_positive(now_ms) or cancelled_status ~= 4 or command_type ~= 3 or schema_version ~= 1 or not exact_positive(terminal_ttl_seconds) or not exact_positive(max_fact_bytes) or not exact_positive(ranking_limit) or ranking_limit > 100 then
  return reject('INVALID_ARGUMENT')
end
